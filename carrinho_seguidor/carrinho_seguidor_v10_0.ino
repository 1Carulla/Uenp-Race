...
#define MOTOR_PWM 150         // Potencia preservada da 9.5 local: 0 a 255.
...
<h1>Seguir a linha preta</h1><p><strong>V10.0 · Pulsos com confirmação da linha</strong></p><p>O carrinho observa a linha, anda um pouco e para para olhar novamente. A diferença entre os motores faz as curvas.</p>
<section><div class="actions"><button id="start" disabled>Iniciar seguidor</button><button id="stop" class="danger">PARAR</button></div>
<p id="notice" role="status">Conectando ao carrinho…</p><small>Se a linha sumir por um instante, conclui apenas o pulso atual e aguarda reencontrá-la por até 3 segundos. Trajeto ambíguo interrompe o pulso. Sem imagem ou comunicação, encerra o ciclo. Mantenha esta página aberta e visível.</small></section>
...
class LineVision {
  constructor(){this.previousX=null;this.gray=null;this.hist=new Uint32Array(256);this.profile=null;this.depth=null;}
  ...
  detect(rgba,w,h,cfg={}){
      ...
      for(let x=1;x<w-1;x++){
        ...
        if(begin>=0&&(!dark||x===w-2)){
          ...
          if(width>=minWidth&&width<=w*0.40&&begin>1&&end<w-2){
            ...
            runs.push({x:center,y,width,localContrast,score:Math.abs(center-expected)+(previousWidth?Math.abs(width-previousWidth)*0.3:0)});
          }
          begin=-1;
        }
      }
      if(!runs.length){
        ...
        for(let x=1;x<w-1;x++){
          ...
          if(start>=0&&(!dark||x===w-2)){
            ...
            if(end-start+1>w*.40&&start<=expected&&end>=expected)
              return fail('Cruzamento ou faixa larga demais',points,true);
            start=-1;
          }
        }
        continue;
    ...
    const confidence=Math.min(1,localContrast/60)*(0.65+0.35*n/7)*Math.max(0,1-residual/(w*0.12))*(limitedNear?0.85:1);
    ...
    return {valid:true,reason:limitedNear||partial?'Linha encontrada; leitura parcial — velocidade reduzida':'Linha encontrada',limitedNear,partial,speedScale:limitedNear||partial?0.65:1,direction,heading,target,curve,points,near,far,error,confidence,threshold,contrast,localContrast,top,bottom};
  }
}
...
function startBlockReason(){
  ...
  if(device.control!=='steps-v10.0')return 'Este painel exige o firmware 10.0 por pulsos.';
...
function openSocket(){
  ...
  socket.onmessage=event=>{
    ...
    if(s.t==='armed'){
      ...
      if(s.mode!==1||s.control!=='steps-v10.0'){stop('Modo de movimento inválido. Recarregue o painel.');return;}
...
function arm(){
  ...
  if(!send({t:'arm',mode:'steps-v10.0'})){armPending=false;controls();return;}
...
String stateJson(const char *type="status",uint32_t seq=0){
  ...
  snprintf(json,sizeof(json),
    ...
    "\"control\":\"steps-v10.0\",\"phase\":\"%s\",\"servoEnabled\":false,\"left\":%d,\"right\":%d,\"commandAge\":%lu,\"remainingMs\":%lu,"
...
esp_err_t wsHandler(httpd_req_t *req){
  ...
  if(strcmp(t,"stop")==0||strcmp(t,"release")==0){
    ...
    const bool isAuto=cJSON_IsString(m)&&strcmp(m->valuestring,"steps-v10.0")==0;
...
void setup(){
  ...
  Serial.printf("\nSEGUIDOR V10.0 PULSOS COM CONFIRMACAO SEM SERVO | Reset=%d (brownout=%d) | Boot=%lu\n",int(esp_reset_reason()),int(ESP_RST_BROWNOUT),(unsigned long)bootId);
  readSavedCrash();
